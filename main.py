...
from loss_functions import NCC, NMI, MILossGaussian
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Neural Implicit Function for a single scan."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        type=int,
        default=[0, 1, 2, 3],
        help="GPU ID following PCI order.",
    )
...
def main(args):
    ...
    device = (
        f"cuda:{config.SETTINGS.GPU_DEVICE}" if torch.cuda.is_available() else "cpu"
    )
    ...
    if config.MODEL.USE_FF:
        ...
        B_gauss = torch.tensor(
            np.random.normal(
                scale=config.FOURIER.FF_SCALE, size=(config.FOURIER.MAPPING_SIZE, 3)
            ),
            dtype=torch.float32,
        ).to(device)
        input_mapper = input_mapping(B=B_gauss, factor=config.FOURIER.FF_FACTOR).to(
            device
        )
    ...
    if config.MODEL.USE_SIREN:
        model = Siren(
            ...
            hidden_features=config.MODEL.HIDDEN_CHANNELS,
            hidden_layers=config.MODEL.NUM_LAYERS,
            first_omega_0=config.SIREN.FIRST_OMEGA_0,
            hidden_omega_0=config.SIREN.HIDDEN_OMEGA_0,
        ...
        model = WireReal(
            ...
            hidden_features=config.MODEL.HIDDEN_CHANNELS,
            hidden_layers=config.MODEL.NUM_LAYERS,
            first_omega_0=config.WIRE.WIRE_REAL_FIRST_OMEGA_0,
            hidden_omega_0=config.WIRE.WIRE_REAL_HIDDEN_OMEGA_0,
            first_s_0=config.WIRE.WIRE_REAL_FIRST_S_0,
            hidden_s_0=config.WIRE.WIRE_REAL_HIDDEN_S_0,
        )
        ...
        if config.MODEL.USE_TWO_HEADS:
            if (
                config.TRAINING.CONTRAST1_ONLY or config.TRAINING.CONTRAST2_ONLY
            ) == True:
    ...
    if config.TRAINING.USE_MI:
        mi_criterion = MILossGaussian(
            num_bins=config.MI_CC.MI_NUM_BINS,
            sample_ratio=config.MI_CC.MI_SAMPLE_RATIO,
            gt_val=config.MI_CC.GT_VAL,
        )
    ...
    if config.TRAINING.USE_NMI:
        mi_criterion = NMI(
            intensity_range=(0, 1),
            nbins=config.MI_CC.MI_NUM_BINS,
            sigma=config.MI_CC.NMI_SIGMA,
        )
        model_name = f'{model_name}_{get_string(config_dict["MI_CC"])}_'
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer=optimizer, T_max=config.TRAINING.EPOCHS
    )
    ...
    for epoch in range(config.TRAINING.EPOCHS):
        ...
        for batch_idx, (data, labels, segm) in enumerate(train_dataloader):
            ...
            if (
                not config.TRAINING.CONTRAST1_ONLY
                and not config.TRAINING.CONTRAST2_ONLY
            ):
            ...
            if config.TRAINING.CONTRAST2_ONLY or config.TRAINING.CONTRAST1_ONLY:
                ...
                if torch.cuda.is_available():
                    data, contrast1_labels, contrast2_labels = (
                        data.to(device=device),
                        contrast1_labels.to(device=device),
                        contrast2_labels.to(device=device),
                    )
            ...
            if (
                config.MODEL.USE_SIREN or config.MODEL.USE_WIRE_REAL
            ):  # TODO check syntax compatibility
                target, _ = target
            ...
            if (
                ...
                mse_target1 = target[
                    : len(contrast1_data), 0:1
                ]  # contrast1 output for contrast1 coordinate
                mse_target2 = target[
                    len(contrast1_data) :, 1:2
                ]  # contrast2 output for contrast2 coordinate
                ...
                if config.MI_CC.MI_USE_PRED:
                    ...
                    config.TRAINING.USE_MI
                    or config.TRAINING.USE_NMI
                    or config.TRAINING.USE_CC
                ):
                    mi_target1 = target[: len(contrast1_data), 1][
                        contrast1_segm.squeeze()
                    ]  # contrast2 output for contrast1 coordinate
                    mi_target2 = target[len(contrast1_data) :, 0][
                        contrast2_segm.squeeze()
                    ]  # contrast1 output for contrast2 coordinate
            ...
            if (
                ...
                loss = config.TRAINING.LOSS_MSE_C1 * criterion(
                    mse_target1, contrast1_labels
                ) + config.TRAINING.LOSS_MSE_C2 * criterion(
            ...
            if config.TRAINING.USE_MI or config.TRAINING.USE_NMI:
                if config.MI_CC.MI_USE_PRED:
                    mi_loss = mi_criterion(
                        mi_target1.unsqueeze(0).unsqueeze(0),
                        mi_target2.unsqueeze(0).unsqueeze(0),
                    )
                    loss += config.MI_CC.LOSS_MI * (mi_loss)
                    if args.logging:
                        wandb_batch_dict.update({"mi_loss": (mi_loss).item()})
                else:
                    mi_loss1 = mi_criterion(
                        mi_target1.unsqueeze(0).unsqueeze(0),
                        contrast1_labels[contrast1_segm].unsqueeze(0).unsqueeze(0),
                    )
                    mi_loss2 = mi_criterion(
                        mi_target2.unsqueeze(0).unsqueeze(0),
                        contrast2_labels[contrast2_segm].unsqueeze(0).unsqueeze(0),
                    )
                    loss += config.MI_CC.LOSS_MI * (mi_loss1 + mi_loss2)
                    if args.logging:
                        wandb_batch_dict.update(
                            {"mi_loss": (mi_loss1 + mi_loss2).item()}
                        )

            if config.TRAINING.USE_CC:
                cc_loss1 = cc_criterion(
                    mi_target1.unsqueeze(0).unsqueeze(0),
                    contrast1_labels[contrast1_segm].unsqueeze(0).unsqueeze(0),
                )
                cc_loss2 = cc_criterion(
                    mi_target2.unsqueeze(0).unsqueeze(0),
                    contrast2_labels[contrast2_segm].unsqueeze(0).unsqueeze(0),
                )
        ...
        if epoch % 5 == 0 or epoch == (config.TRAINING.EPOCHS - 1):
            ...
            for batch_idx, (data) in enumerate(infer_loader):
                ...
                # out[batch_idx * 5000 : (batch_idx * 5000 + len(output)), :] = (
                ...
                # 使用动态索引进行赋值
                out[current_idx : current_idx + num_samples, :] = (
                    output.cpu().detach().numpy()
                )
            ...
            if (
                ...
                model_intensities_contrast1 = scaler.fit_transform(
                    label_arr.reshape(-1, 1)
                )

                label_arr = np.array(model_intensities_contrast2, dtype=np.float32)
                model_intensities_contrast2 = scaler.fit_transform(
                    label_arr.reshape(-1, 1)
                )
                ...
                gt_contrast1 = (
                    dataset.get_contrast1_gt()
                    .reshape((x_dim, y_dim, z_dim))
                    .cpu()
                    .numpy()
                )
                gt_contrast2 = (
                    dataset.get_contrast2_gt()
                    .reshape((x_dim, y_dim, z_dim))
                    .cpu()
                    .numpy()
                )
                ...
                gt_contrast1 = scaler.fit_transform(
                    gt_contrast1.reshape(-1, 1)
                ).reshape((x_dim, y_dim, z_dim))

                label_arr = np.array(gt_contrast2, dtype=np.float32)
                gt_contrast2 = scaler.fit_transform(
                    gt_contrast2.reshape(-1, 1)
                ).reshape((x_dim, y_dim, z_dim))
                ...
                metrics_contrast1 = compute_metrics(
                    gt=gt_contrast1.copy(),
                    pred=pred_contrast1.copy(),
                    mask=dataset.get_contrast1_gt_mask(),
                    lpips_loss=lpips_loss,
                    device=device,
                )
                metrics_contrast2 = compute_metrics(
                    gt=gt_contrast2.copy(),
                    pred=pred_contrast2.copy(),
                    mask=dataset.get_contrast2_gt_mask(),
                    lpips_loss=lpips_loss,
                    device=device,
                )

                metrics_mi_true = compute_mi_hist(
                    gt_contrast1.copy(),
                    gt_contrast2.copy(),
                    dataset.get_contrast2_gt_mask(),
                    bins=32,
                )
                metrics_mi_1 = compute_mi_hist(
                    pred_contrast1.copy(),
                    gt_contrast2.copy(),
                    dataset.get_contrast2_gt_mask(),
                    bins=32,
                )
                metrics_mi_2 = compute_mi_hist(
                    pred_contrast2.copy(),
                    gt_contrast1.copy(),
                    dataset.get_contrast2_gt_mask(),
                    bins=32,
                )
                metrics_mi_pred = compute_mi_hist(
                    pred_contrast1.copy(),
                    pred_contrast2.copy(),
                    dataset.get_contrast2_gt_mask(),
                    bins=32,
                ...
                if args.logging:
                    wandb_epoch_dict.update(
                        {f"contrast1_ssim": metrics_contrast1["ssim"]}
                    )
                    wandb_epoch_dict.update(
                        {f"contrast1_psnr": metrics_contrast1["psnr"]}
                    )
                    wandb_epoch_dict.update(
                        {f"contrast1_lpips": metrics_contrast1["lpips"]}
                    )
                    wandb_epoch_dict.update(
                        {f"contrast2_ssim": metrics_contrast2["ssim"]}
                    )
                    wandb_epoch_dict.update(
                        {f"contrast2_psnr": metrics_contrast2["psnr"]}
                    )
                    wandb_epoch_dict.update(
                        {f"contrast2_lpips": metrics_contrast2["lpips"]}
                    )
                    wandb_epoch_dict.update(
                        {f"mutual_information": metrics_mi_pred["mi"]}
                    )
                    wandb_epoch_dict.update(
                        {f"mutual_information_appox": metrics_mi_approx["mi"]}
                    )
                    wandb_epoch_dict.update(
                        {
                            f"MI_error_contrast1": np.abs(
                                metrics_mi_1["mi"] - metrics_mi_true["mi"]
                            )
                        }
                    )
                    wandb_epoch_dict.update(
                        {
                            f"MI_error_contrast2": np.abs(
                                metrics_mi_2["mi"] - metrics_mi_true["mi"]
                            )
                        }
                    )
                    wandb_epoch_dict.update(
                        {
                            f"MI_error_pred": np.abs(
                                metrics_mi_pred["mi"] - metrics_mi_true["mi"]
                            )
                        }
                    )
                ...
                if epoch == (config.TRAINING.EPOCHS - 1):
                    nib.save(
                        img,
                        os.path.join(patient_image_dir, "T1.nii.gz"),
                    )
                ...
                if epoch == (config.TRAINING.EPOCHS - 1):
                    nib.save(
                        img,
                        os.path.join(patient_image_dir, "T2.nii.gz"),
                    )
                ...
                im = show_slices_gt(
                    [slice_0, slice_1, slice_2], [bslice_0, bslice_1, bslice_2], epoch
                )
                if args.logging:
                    image = wandb.Image(
                        im, caption=f"{config.DATASET.LR_CONTRAST1} prediction vs gt."
                    )
                ...
                if args.logging:
                    image = wandb.Image(
                        im, caption=f"{config.DATASET.LR_CONTRAST2} prediction vs gt."
                    )
                ...
                curr_mean = np.mean(
                    np.abs(mi_buffer[:-1] - mi_buffer[1:])
                )  # srtore diff of abs change
                ...
                gt = scaler.fit_transform(gt.reshape(-1, 1)).reshape(
                    (x_dim, y_dim, z_dim)
                )
                ...
                metrics_mi_true = compute_mi_hist(
                    gt.copy(), gt_other.copy(), dataset.get_contrast2_gt_mask(), bins=32
                )
                metrics_mi = compute_mi_hist(
                    pred.copy(),
                    gt_other.copy(),
                    dataset.get_contrast2_gt_mask(),
                    bins=32,
                ...
                if args.logging:
                    if config.TRAINING.CONTRAST2_ONLY:
                        ...
                        wandb_epoch_dict.update(
                            {
                                f"MI_error_contrast2": np.abs(
                                    metrics_mi["mi"] - metrics_mi_true["mi"]
                                )
                            }
                        )
                ...
                if config.TRAINING.CONTRAST2_ONLY:
                    nifti_name = "T2.nii.gz"  # model_name_epoch.replace("model.pt", f"_ct2.nii.gz")
                else:
                    nifti_name = "T1.nii.gz"  # model_name_epoch.replace("model.pt", f"_ct1.nii.gz")
                ...
                if args.logging:
                    if config.TRAINING.CONTRAST2_ONLY:
                        image = wandb.Image(
                            im,
                            caption=f"{config.DATASET.LR_CONTRAST2} prediction vs gt.",
                        )
                        wandb_epoch_dict.update(
                            {f"{config.DATASET.LR_CONTRAST2}": image}
                        )
                    else:
                        image = wandb.Image(
                            im,
                            caption=f"{config.DATASET.LR_CONTRAST1} prediction vs gt.",
                        )
                        wandb_epoch_dict.update(
                            {f"{config.DATASET.LR_CONTRAST1}": image}
                        )
