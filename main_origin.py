...
def main(args):
    ...
    for epoch in range(config.TRAINING.EPOCHS):
        ...
        if not config.TRAINING.CONTRAST1_ONLY and not config.TRAINING.CONTRAST2_ONLY:
            ...
            gt_contrast1 = (
                dataset.get_contrast1_gt().reshape((x_dim, y_dim, z_dim)).cpu().numpy()
            )
            gt_contrast2 = (
                dataset.get_contrast2_gt().reshape((x_dim, y_dim, z_dim)).cpu().numpy()
            )

            label_arr = np.array(gt_contrast1, dtype=np.float32)
            gt_contrast1 = scaler.fit_transform(gt_contrast1.reshape(-1, 1)).reshape(
                (x_dim, y_dim, z_dim)
            )

            label_arr = np.array(gt_contrast2, dtype=np.float32)
            gt_contrast2 = scaler.fit_transform(gt_contrast2.reshape(-1, 1)).reshape(
                (x_dim, y_dim, z_dim)
            )
            ...
            if epoch == (config.TRAINING.EPOCHS - 1):
                nib.save(
                    img,
                    os.path.join(
                        image_dir, model_name_epoch.replace("model.pt", f"_ct1.nii.gz")
                    ),
                )
            ...
            if epoch == (config.TRAINING.EPOCHS - 1):
                nib.save(
                    img,
                    os.path.join(
                        image_dir, model_name_epoch.replace("model.pt", f"_ct2.nii.gz")
                    ),
                )
            ...
            metrics_mi = compute_mi_hist(
                pred.copy(), gt_other.copy(), dataset.get_contrast2_gt_mask(), bins=32
            )
